...
class SearchRequest(BaseModel):
    ...
    query: str = Field(
                default="",
                description="Text to search for (optional if image_base64 "
                "is provided)",
    )
    image_base64: str = Field(
                default="",
                description="Base64 encoded image string (without "
...
app = FastAPI(
        ...
        description=(
                    "Multilingual semantic search over EuroSAT imagery"
        ),
        version="0.5.0",
        lifespan=lifespan,
)
...
async def search(request: SearchRequest):
    ...
    if request.image_base64:
                try:
                                ...
                                pil_image = Image.open(BytesIO(image_data)).convert(
                                    "RGB"
                                )
